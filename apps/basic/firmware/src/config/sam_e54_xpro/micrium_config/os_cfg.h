...
#define OS_CFG_FLAG_PEND_ABORT_EN         DEF_ENABLED         /*     Include (DEF_ENABLED) code for OSFlagPendAbort()                   */
                                          ...
                                          /* -------------------------- MEMORY MANAGEMENT ------------------------ */
#define OS_CFG_MEM_EN                     DEF_ENABLED                     /* Enable (DEF_ENABLED) code generation for MEMORY MANAGER                */
...
#define OS_CFG_SEM_DEL_EN                 DEF_ENABLED                 /*    Include code (DEF_ENABLED) for OSSemDel()                           */
#define OS_CFG_SEM_PEND_ABORT_EN          DEF_ENABLED          /*    Include code (DEF_ENABLED) for OSSemPendAbort()                     */
#define OS_CFG_SEM_SET_EN                 DEF_ENABLED                 /*    Include code (DEF_ENABLED) for OSSemSet()                           */


                                          /* -------------------------- TASK MANAGEMENT -------------------------- */
#define OS_CFG_STAT_TASK_EN               DEF_ENABLED               /* Enable (DEF_ENABLED) the statistics task                               */
#define OS_CFG_STAT_TASK_STK_CHK_EN       DEF_ENABLED       /*    Check task stacks from statistic task                               */

#define OS_CFG_TASK_CHANGE_PRIO_EN        DEF_ENABLED        /* Include code (DEF_ENABLED) for OSTaskChangePrio()                      */
#define OS_CFG_TASK_DEL_EN                DEF_ENABLED                /* Include code (DEF_ENABLED) for OSTaskDel()                             */
#define OS_CFG_TASK_IDLE_EN               DEF_ENABLED               /* Include code (DEF_ENABLED) the idle task                               */
#define OS_CFG_TASK_PROFILE_EN            DEF_ENABLED            /* Include variables in OS_TCB for profiling                              */
#define OS_CFG_TASK_Q_EN                  DEF_ENABLED                  /* Include code (DEF_ENABLED) for OSTaskQXXXX()                           */
#define OS_CFG_TASK_Q_PEND_ABORT_EN       DEF_ENABLED       /* Include code (DEF_ENABLED) for OSTaskQPendAbort()                      */
...
#define OS_CFG_TASK_SEM_PEND_ABORT_EN     DEF_ENABLED     /* Include code (DEF_ENABLED) for OSTaskSemPendAbort()                    */
#define OS_CFG_TASK_SUSPEND_EN            DEF_ENABLED            /* Include code (DEF_ENABLED) for OSTaskSuspend() and OSTaskResume()      */
...
#define OS_CFG_TLS_TBL_SIZE               0u                                                             /* Include (DEF_ENABLED) code for Task Local Storage (TLS) registers      */
                                          ...
                                          /* -------------------------- TIME MANAGEMENT -------------------------- */
#define OS_CFG_TIME_DLY_HMSM_EN           DEF_ENABLED           /*     Include code (DEF_ENABLED) for OSTimeDlyHMSM()                     */
#define OS_CFG_TIME_DLY_RESUME_EN         DEF_ENABLED         /*     Include code (DEF_ENABLED) for OSTimeDlyResume()                   */
